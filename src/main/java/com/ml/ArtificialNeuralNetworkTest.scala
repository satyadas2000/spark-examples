...
object ArtificialNeuralNetworkTest {
			...
			//	df=df.na.fill(avgApplicantIncome, Seq("ApplicantIncome"))
				...
				var avgLoanAmount = df.select(mean("LoanAmount")).first()(0).asInstanceOf[Double]
						...
						var avgLoan_Amount_Term = df.select(mean("Loan_Amount_Term")).first()(0).asInstanceOf[Double]
								...
								val layers = Array[Int](9,  28, 25, 2)   					
								//9- is number of features , 2 is no of classes predicted (here loan_status Y, N)
								//hiddenLayerOpts - 2 hidden layers consist of 28, 25 computational units respectively

								val ann = new MultilayerPerceptronClassifier()
		                .setSeed(1234L).setLabelCol("label").setFeaturesCol("features").setLayers(layers)
                    .setBlockSize(128)
                    .setMaxIter(100)
